...
class SchoolStudent(models.Model):
    ...
    def _get_spp_product(self):
        spp_product_id = int(self.env['ir.config_parameter'].sudo().get_param('school_management.default_spp_product_id'))
        if not spp_product_id:
            spp_product_id = self.env['product.product'].create(
                self._prepare_spp_product_values()
            ).id
            self.env['ir.config_parameter'].sudo().set_param('school_management.default_spp_product_id', spp_product_id)
